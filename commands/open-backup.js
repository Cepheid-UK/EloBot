...
exports.run = (client, message, args, connection) => {
    ...
    connection.query('SELECT * FROM players WHERE discord_id= ?', [message.author.tag], function (err, results) {
        ...
        if (results.length === 0) {
            // no player found in the 'players' table
            message.channel.send('Please sign up for the EloBot Ladder using the ``!signup`` command')
        } else {
            // player found
                    ...
                    connection.query(sql_challenge, async function (err, result) {
                        ...
                        if (queryResult2.length === 0) {
                            ...
                            connection.query(sql_createOpenChallenge, async function (err, result) {
                                ...
                                // condition here required to make sure someone who is not a part of the elo league cannot sign up to this game
                                async function filter(reaction, user) {
                                    if (reaction.emoji.name == '✅' && user.tag != discord_id) {
                                        return false;
                                    }
                                    await connection.query('SELECT * FROM players WHERE discord_id=?', [user.tag], function (err, results) {
                                        if (results.length === 0) {
                                            // player isn't in the db 
                                            console.log('returned false')
                                            return false;
                                        } else {
                                            // player is in the db, not the author, not the bot, and reacted with a ✅
                                ...
                                console.log('starting awaitReactions')
                                sentEmbedMessage.awaitReactions((reaction, user) => 
                                    filter(reaction, user) && user.id != sentEmbedMessage.author.id, {max: 1, time: CHALLENGE_TIMER}).then(collected => {
                                        ...
                                        // select a number between 1-7 for the map
                                        sql_mapSelection = 'SELECT * FROM mappool WHERE id=' + Math.floor(Math.random()*7+1)
